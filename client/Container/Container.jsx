...
export class Container extends React.Component {
  ...
  render() {
    const Markers = this.state.markers.map((marker, index) => (<Marker key={index} onClick={this.onMarkerClick} company={marker['company']} jobtitle={marker['jobtitle']} snippet={marker['snippet']} url={marker['url']} jobkey={marker['jobkey']} position={{
      lat: marker['lat'],
      lng: marker['lng']
    }}/>));
        ...
        <div className='overallContainer'>
          ...
          <div className='mapContainer'>
            <Map google={this.props.google} style={{
              ...
              height: '100%',
              position: 'relative'
            }} className={'map'} zoom={14} onClick={this.onMapClicked} onDragend={this.onMapMoved}>
              {Markers}
              <InfoWindow marker={this.state.activeMarker} visible={this.state.showingInfoWindow} onClose={this.onInfoWindowClose}>
                <div>
                  ...
                  <div>
                    <a href={this.state.selectedPlace.url}>Click to View</a>
                  </div>
                </div>
              </InfoWindow>
            </Map>
...
export default GoogleApiWrapper({apiKey: process.env.GOOGLE_MAPS_API_KEY})(Container);
